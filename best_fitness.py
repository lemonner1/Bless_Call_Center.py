...
def evaluate(individual):
    ...
    for shift in range(NUM_SHIFTS):
        ...
        if not can_perform_shift(operator, shift):
            invalid_skill_penalty += 1

    for shifts in shifts_per_operator:
        if shifts > MAX_SHIFTS_PER_OPERATOR:
            overload_penalty += shifts - MAX_SHIFTS_PER_OPERATOR

    return (preference_score, -invalid_skill_penalty, -overload_penalty, -duplicate_shift_penalty)
...
def main():
    ...
    for gen in range(NGEN):
        ...
        print(f"Поколение {gen}: Лучший фитнес: {best_ind.fitness.values}")
